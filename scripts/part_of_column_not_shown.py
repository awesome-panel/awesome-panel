...
class Correction(param.Parameterized):
    ...
    def _citaj_baza(self):
        ...
        if self.kriterium != "%%":
            ...
            """.format(
                pole=self.pole, kriterium=self.kriterium
            )
    ...
    def _add_row(self):
        ...
        if len(self.sk) > 0:
            ...
            for index, self.row in self.sk.iterrows():
                ...
                # Za doma
                patek = pn.widgets.LiteralInput(
                    value=self.row["PATEKAOBJAVA"]
                    .strip()
                    .replace(
                        "C:\\inetpub\\wwwroot\\PXWeb\\Resources\\PX\\Databases\\MakStat\\", ""
                    ),
                    disabled=True,
                    width=400,
                )
                datumobj = pn.widgets.DatePicker(
                    name="Датум на објавување на веб",
                    value=datetime.strptime(self.row["DATUMOBJAVA"], "%Y-%m-%d").date(),
                    width=150,
                )
                koris = pn.widgets.LiteralInput(
                    value=self.row["KORISNIK"].strip(), disabled=True, width=250
                )
                datumvneseno = pn.widgets.LiteralInput(
                    value=datetime.strftime(self.row["DATUMAZUR"], "%d-%m-%Y"),
                    disabled=True,
                    width=100,
                )
    ...
    def view(self):
        ...
        return pn.pane.LaTeX(
            ("{user}".format(user=self.user)), style={"font-size": "18pt"}, width=800
        )
    ...
    def panel(self):
        ...
        pnl = pn.Column(
            ...
            pn.Param(
                self,
                widgets={
                    "rad": {"type": pn.widgets.RadioButtonGroup, "button_type": "success"},
                    "sel": {
                        "type": pn.widgets.Select,
                        "options": [i.parts[8] for i in list(self.oblast1.keys())],
                    },
                    "action": {"type": pn.widgets.Button(button_type="primary")},
                },
            ),

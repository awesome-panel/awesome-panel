"""In this module we define all Authors of resources or apps in the gallery and exposes
them via the AUTHORS list"""
from awesome_panel.application.models import Author

# Authors
ANDREW_HUANG = Author(
    name="Andrew Huang",
    url="https://twitter.com/iateandrew1",
    github_url="https://github.com/ahuang11",
    github_avatar=(
        "https://avatars3.githubusercontent.com/u/"
        "15331990?s=400&u=6f855e2f82d576fd22110f2ac9b1b7ec4825d916&v=4"
    ),
)
AWESOME_PANEL_ORG = Author(
    name="Awesome-panel.org",
    url="https://awesome-panel.org",
    github_url="https://github.com/marcskovmadsen",
    github_avatar="https://avatars0.githubusercontent.com/u/24277277?s=400&v=4",
)
BENJAMIN_COOLEY = Author(
    name="Bokeh",
    url="https://bendoesdata.github.io/",
    github_url="https://github.com/bendoesdata",
    github_avatar="https://avatars3.githubusercontent.com/u/8440965",
)
BOKEH = Author(
    name="Bokeh",
    url="https://docs.bokeh.org/en/latest/ ",
    github_url="https://github.com/bokeh",
    github_avatar="https://avatars3.githubusercontent.com/u/8440965",
)
DASH = Author(
    name="Dash",
    url="https://plot.ly/dash/",
    github_url="https://github.com/plotly/dash",
    github_avatar="https://avatars2.githubusercontent.com/u/5997976",
)
HAYLEY_SONG = Author(
    name="Hayley Song",
    url="http://holoviz.org/",
    github_url="https://github.com/cocoaaa",
    github_avatar="https://avatars0.githubusercontent.com/u/8037983",
)
HOLOVIZ = Author(
    name="HoloViz",
    url="http://holoviz.org/",
    github_url="https://github.com/holoviz",
    github_avatar="https://avatars2.githubusercontent.com/u/51678735",
)
INFORMATIONISBEUTIFULL = Author(
    name="Information is beautiful",
    url="https://informationisbeautiful.net/",
    github_url="",
    github_avatar="",
)
INTAKE = Author(
    name="Intake",
    url="https://github.com/intake",
    github_url="https://github.com/intake",
    github_avatar="https://avatars1.githubusercontent.com/u/1469464",
)
JAMES_BEDNAR = Author(
    name="James A. Bednar",
    url="https://github.com/jbednar",
    github_url="https://github.com/jbednar",
    github_avatar="https://avatars0.githubusercontent.com/u/1695496",
)
JOCHEM_SMIT = Author(
    name="Jochem Smit",
    url="https://github.com/Jhsmit",
    github_url="https://github.com/Jhsmit",
    github_avatar=(
        "https://avatars1.githubusercontent.com/u/"
        "7881506?s=400&u=bdf7b6635bf57e7022763ce3b002649fe80ef6a8&v=40"
    ),
)
JUSTIN_BOIS = Author(
    name="Justin Bois",
    url="http://bois.caltech.edu/",
    github_url="https://github.com/JUSTIN_BOIS",
    github_avatar="https://avatars3.githubusercontent.com/u/1873517",
)
LEON_VAN_KOUWEN = Author(
    name="Leon van Kouwen",
    url="https://github.com/LeonvanKouwen",
    github_url="https://github.com/LeonvanKouwen",
    github_avatar="https://avatars3.githubusercontent.com/u/6023630",
)
MARC_SKOV_MADSEN = Author(
    name="Marc Skov Madsen",
    url="https://datamodelsanalytics.com",
    github_url="https://github.com/marcskovmadsen",
    github_avatar="https://avatars0.githubusercontent.com/u/42288570",
)
MATTEO_NICCOLI = Author(
    name="Matteo Niccoli",
    url="https://twitter.com/My_Carta",
    github_url="https://github.com/mycarta",
    github_avatar="https://avatars3.githubusercontent.com/u/7424763",
)
MATTEO_Mancini = Author(
    name="Matteo Mancini",
    url="https://neurosnippets.com/",
    github_url="https://github.com/matteomancini",
    github_avatar="https://avatars.githubusercontent.com/u/29856041",
)
NIC_FOX = Author(
    name="Nic Fox",
    url="https://dev.to/nicfoxds",
    github_url="https://github.com/foxnic",
    github_avatar="https://avatars3.githubusercontent.com/u/39113566",
)
OGGM_EDU = Author(
    name="OGGM EDU",
    url="https://edu.oggm.org/en/latest/index.html",
    github_url="https://github.com/OGGM",
    github_avatar="https://avatars2.githubusercontent.com/u/14758186",
)
OURWORLDINDATA = Author(
    name="Our World in Data",
    url="https://ourworldindata.org/",
    github_url="https://github.com/owid",
    github_avatar="https://avatars3.githubusercontent.com/u/14187135",
)
PANEL = Author(
    name="panel",
    url="https://panel.pyviz.org/",
    github_url="https://github.com/holoviz/",
    github_avatar="https://avatars2.githubusercontent.com/u/51678735",
)
PIERRE_OLIVIER_SIMONARD = Author(
    name="Pierre-Olivier Simonard",
    url="https://www.linkedin.com/in/pierreoliviersimonard/?originalSubdomain=fr",
    github_url="https://github.com/pierrotsmnrd",
    github_avatar="https://avatars.githubusercontent.com/u/756464?v=4",
)
PHILIPP_RUDIGER = Author(
    name="Philipp Rudiger",
    url="http://philippjfr.com/",
    github_url="https://github.com/philippjfr",
    github_avatar="https://avatars0.githubusercontent.com/u/1550771",
)
PLOTLY = Author(
    name="Plotly",
    url="https://plot.ly/",
    github_url="https://github.com/plotly",
    github_avatar="https://avatars2.githubusercontent.com/u/5997976",
)
PYGRIDGEN = Author(
    name="PyGridGen",
    url="https://github.com/pygridgen",
    github_url="https://github.com/pygridgen",
    github_avatar="https://avatars1.githubusercontent.com/u/36489971",
)
QUANSIGHT = Author(
    name="Quansight",
    url="https://www.quansight.com/",
    github_url="https://github.com/Quansight",
    github_avatar="https://avatars1.githubusercontent.com/u/34879953",
)
STREAMLIT = Author(
    name="Streamlit",
    url="https://streamlit.io/",
    github_url="https://github.com/streamlit",
    github_avatar="https://avatars1.githubusercontent.com/u/45109972",
)
THOMAS_PEDOT = Author(
    name="Thomas PEDOT",
    url="https://github.com/slamer59",
    github_url="https://github.com/slamer59",
    github_avatar="https://avatars2.githubusercontent.com/u/25283272",
)
VDA_LAB = Author(
    name="VDA Lab",
    url="http://vda-lab.github.io/",
    github_url="https://github.com/vda-lab",
    github_avatar="https://avatars3.githubusercontent.com/u/13289096",
)
VOILA = Author(
    name="Voila",
    url="https://github.com/voila-dashboards",
    github_url="https://github.com/voila-dashboards",
    github_avatar="https://avatars1.githubusercontent.com/u/55792893?s=200&v=4",
)
DAMIAN_FARRELL = Author(
    name="Damien Farrell",
    url="https://dmnfarrell.github.io/",
    github_url="https://github.com/dmnfarrell",
    github_avatar="https://avatars0.githubusercontent.com/u/7859189",
)
XAVARTLEY = Author(
    name="xavArtely",
    url="https://github.com/xavArtley",
    github_url="https://github.com/xavArtley",
    github_avatar="https://avatars1.githubusercontent.com/u/18531147",
)


AUTHORS = [
    ANDREW_HUANG,
    AWESOME_PANEL_ORG,
    BOKEH,
    DASH,
    DAMIAN_FARRELL,
    HAYLEY_SONG,
    HOLOVIZ,
    INTAKE,
    JAMES_BEDNAR,
    MARC_SKOV_MADSEN,
    NIC_FOX,
    OGGM_EDU,
    OURWORLDINDATA,
    PANEL,
    PIERRE_OLIVIER_SIMONARD,
    PHILIPP_RUDIGER,
    PYGRIDGEN,
    QUANSIGHT,
    PLOTLY,
    STREAMLIT,
    THOMAS_PEDOT,
    VDA_LAB,
    VOILA,
    XAVARTLEY,
]

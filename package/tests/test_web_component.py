# pylint: disable=redefined-outer-name,protected-access,abstract-method, unused-argument
...
def html_escaped():
    return (
        "&lt;wired-radio id=&quot;1&quot; string=&quot;&quot; integer=&quot;0&quot; "
        "number=&quot;0.0&quot;&gt;Radio Two&lt;/wired-radio&gt;"
    )
...
def test_constructor_with_parameter_values(CustomWebComponent):
    ...
    component = CustomWebComponent(
        html=html, boolean=boolean, string=string, integer=integer, number=number
    )
...
def test_bokeh_model(document, comm, html, html_escaped):
    # Given
...
def test_web_component_parameter_attribute_change(document, comm, CustomWebComponent):
    ...
    assert component.boolean is False
    ...
    assert component.list_ is None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '<wired-radio boolean id="3" boolean>Radio Two</wired-radio>',
            {"boolean": None, "id": "3"},
        ),
...
def test_update_parameters_boolean(CustomWebComponent):
    ...
    assert custom_web_component.boolean is False
    custom_web_component.html = "<wired-radio boolean>Radio Two</wired-radio>"
    assert custom_web_component.boolean is True
    custom_web_component.html = "<wired-radio>Radio Two</wired-radio>"
    assert custom_web_component.boolean is False
    custom_web_component.html = '<wired-radio boolean="">Radio Two</wired-radio>'
    assert custom_web_component.boolean is True
...
def test_update_parameters_list():
    ...
    assert (
        component.html == "<a></a>"
    )  # We don't update the html because it would send it to client
...
def test_parameter_change_to_attribute_change(CustomWebComponent):
    ...
    html = (
        '<wired-radio id="1" string="" integer="0" number="0.0" boolean="">Radio Two</wired-radio>'
    )
    assert component.html == html
    ...
    component.update_html_from_attributes_to_watch()
    html = (
        '<wired-radio id="1" string="a" integer="1" number="1.1" boolean="">Radio Two</wired-radio>'
    )
    assert component.html == html
...
def test_property_change_triggers_parameter_change(CustomWebComponent):
    # Given
    component = CustomWebComponent(
        attributes_to_watch={}, properties_to_watch={"boolean": "boolean"}
    )
...
def test_handle_attributes_to_watch_change_on_html_with_leading_new_line_and_spaces():
    ...
    class Custom(WebComponent):
        html = param.String(
            """
        <perspective-viewer id="view1" class='perspective-viewer-material-dark' \
    ...
    assert component.html.startswith(
        """<perspective-viewer id="view1" class="perspective-viewer-material" \
style="height:100%;width:100%"></perspective-viewer>"""
    )

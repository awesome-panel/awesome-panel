...
select_city = pn.widgets.Select(
    name="City",
    options=CITIES[select_continent.value],  # type: ignore
    value=CITIES[select_continent.value][0],  # type: ignore
).servable(area="sidebar")
select_city_auto = pn.widgets.AutocompleteInput(
    name="City", options=CITIES_LIST, value=select_city.value  # type: ignore
).servable(area="sidebar")
...
@pn.depends(select_city.param.value, watch=True)  # type: ignore
def _update_plot(*_):

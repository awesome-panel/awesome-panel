...
def get_plot(theme="default"):
    ...
    # facets.
    cylinder = vtk.vtkCylinderSource()  # pylint: disable=no-member
    cylinder.SetResolution(8)

    # The mapper is responsible for pushing the geometry into the graphics
    # library. It may also do color mapping, if scalars or other
    # attributes are defined.
    cylinder_mapper = vtk.vtkPolyDataMapper()  # pylint: disable=no-member
    cylinder_mapper.SetInputConnection(cylinder.GetOutputPort())

    # The actor is a grouping mechanism: besides the geometry (mapper), it
    # also has a property, transformation matrix, and/or texture map.
    # Here we set its color and rotate it -22.5 degrees.
    cylinder_actor = vtk.vtkActor()  # pylint: disable=no-member
    ...
    # window.
    render = vtk.vtkRenderer()  # pylint: disable=no-member
...
component = pn.pane.VTK(plot, height=700, sizing_mode="stretch_both").servable()  # type: ignore

...
class LoadingStyler(param.Parameterized):
    ...
    spinner = param.ObjectSelector(
        default=DEFAULT_URL,
        objects=loading_spinners_config.SPINNERS,
    ...
    color = param.Color(loading_spinners_config.DEFAULT_COLOR)
...
if __name__.startswith("bokeh"):
    config.extension(url="loading_spinners")
    app = LoadingApp(name="Loading Spinner App")

    app.settings_panel.servable(area="sidebar")
    app.main.servable()

    if not pn.state.template or not issubclass(
        pn.state.template.theme, pn.template.base.DefaultTheme
    ):
